...
[![Build Status](https://travis-ci.org/piemasters/Portfolio.svg?branch=master)](https://travis-ci.org/piemasters/New-Portfolio)
...
* [Bootstrap](https://getbootstrap.com/) - Components & Styling
...
3. From a terminal run `nvm use 10.12.0` to enable NodeJS
...
To access the running application navigate to `http://localhost:4200/`.
...
## Styling
[Bootstrap](https://getbootstrap.com/) is used for the components and styling used throughout the app.
...
The build artifacts will be stored in the `dist/portfolio-server` directory. 
...
* [start-server-and-test](https://github.com/bahmutov/start-server-and-test) starts the server, waits for the URL, then runs test command; when the tests end it shuts down the server
